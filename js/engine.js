...
var Engine = (function(global) {
    ...
    // Check with player crosses paths with Enemies
    function checkCollisions() {
      // Check Collision with Enemeies
      allEnemies.forEach(function(enemy) {
        if ((Math.abs(player.x-enemy.x)<70) && (Math.abs(player.y-enemy.y)<10)) {
            player.x = 200;
            player.y = 300;
            player.rowNum = 2;
            player.colNum = 3;
    ...
    function checkGameOver() {
      if (player.numLives < 1) {
        gameOver = true;
        ctx.fillText("GAME OVER", canvas.length/2,canvas.height/2)
        init();
      }
    }
